...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  ...
  case I2C__STATE_SLAVE_RECEIVE_READ_ACK:
    // printf("At state 0x60\n");
    // fprintf(stderr, "FEFF1: %x\n", i2c__read_address(i2c->slave_address));
    // fprintf(stderr, "FEFF1: %d\n", i2c__write_address(i2c->slave_address));
    check = true;
    rand_check = 0;
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    i2c->number_of_bytes_to_transfer = 1;
    break;

  case I2C__STATE_SLAVE_RECEIVE_READ_NACK:
    // printf("At state 0x68\n");
    lpc_i2c->CONSET = 0x24;
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    i2c->number_of_bytes_to_transfer = 1;
    break;
  ...
  case I2C__STATE_SLAVE_RECEIVE_ACK_SENT:
    // printf("At state 0x80\n");
    // *(i2c->input_byte_pointer) = lpc_i2c->DAT; // Go back
    // fprintf(stderr, "Addr: %d\n", i2c->starting_slave_memory_address);
    // fprintf(stderr, "DAT80: %d\n", lpc_i2c->DAT);
    // i2c->count_mem_index = 0;
    rand_check++;
    fprintf(stderr, "DAT: %d\n", lpc_i2c->DAT);
    if (rand_check >= 2) {
      i2c_slave_callback__write_memory_h(i2c->count_mem_index++, lpc_i2c->DAT);
    } else {
      i2c->count_mem_index = lpc_i2c->DAT;
    }
    --(i2c->number_of_bytes_to_transfer);
    if (check) {
      lpc_i2c->CONCLR = 0x0C;
    }
    // if (i2c->number_of_bytes_to_transfer == 0) { // if it 1
    // lpc_i2c->CONCLR = 0x0C;
    // }
    else {
      i2c__set_ack_flag(lpc_i2c);
  ...
  case I2C__STATE_SLAVE_TRANSMIT_READ_ACK:
    // lpc_i2c->DAT = *(i2c->output_byte_pointer);
    // fprintf(stderr, "DATA8: %d\n", lpc_i2c->DAT);
    // i2c->count_mem_index = 0;
    // fprintf(stderr, "DAT: %d\n", lpc_i2c->DAT);

    i2c_slave_callback__read_memory_h(i2c->count_mem_index++, &lpc_i2c->DAT);
  ...
  case I2C__STATE_SLAVE_TRANSMIT_ACK_SENT:
    ...
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    // ++(i2c->output_byte_pointer); // Check datasheet to confirm
    break;
